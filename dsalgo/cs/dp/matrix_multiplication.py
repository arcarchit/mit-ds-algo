def again(dims):
	...
	a = dims[:-1]  # 10,20,50,30
	b = dims[1:]  # 20,50,30,20
	BIG_NO = 10 ** 8
	...
	def sub_sol(start, end):
		...
		if start + 1 == end:
			...
			for k in range(start + 1, end):
				# k will belong to right hand matrix
				left, right = sub_sol(start, k), sub_sol(k, end)
				temp = left + a[start] * a[k] * b[end-1] + right
				ans = min(ans, temp)
		memo[key] = ans
		return ans

	return sub_sol(0, len(a))
